...
class MoonlinkNodes {
  ...
  idealNode() {
    if (!idealNode) {
			let node = Object.values(Nodes).filter((x) => x.ws._readyState === 1).sort((b, a) => a.stats.cpu ? (a.stats.cpu.systemLoad / a.stats.cpu.cores) * 100 : 0 - b.stats.cpu ? (b.stats.cpu.systemLoad / b.stats.cpu.cores) * 100 : 0)[0]
            ...
            this.manager.emit('playerUpdate', data)
                ...
                if (players[data.guildId].loop === 1) {
                  if (!utils.track.current())
                    throw new TypeError(`[ MoonLinkJs ]: cannot loop a music with queue is empty!`)
                ...
                if (players[data.guildId].loop > 1) {
                  ...
                  if (!trackshifted)
                    throw new TypeError(`[ MoonlinkJs ]: queue no has track!`)
  ...
  get(identify) {
    ...
    if (typeof identify == 'Number') node = Object.values(Nodes)
    else {
