...
export class MoonlinkNode {
  ...
  public rest: MoonlinkRest;
	...
	public async connect(): Promise<any> {
		if(typeof this.host !== 'string' && typeof this.host !== 'undefined') throw new Error('[ @Moonlink/Node ]: "host" option is not configured correctly')
		if(typeof this.password !== 'string' && typeof this.password !== 'undefined') throw new Error('[ @Moonlink/Node ]: the option "password" is not set correctly')
		if(this.port && typeof this.port !== 'number' || this.port > 65535 || this.port < 0) throw new Error('[ @Moonlink/Node ]: the "port" option is not set correctly') 
   this.options.clientName ? this.options.clientName : this.options.clientName = `Moonlink/${this.manager.version}`
		this.version = await makeRequest(`http${this.secure ? `s` : ``}://${this.host}${this.port ? `:${this.port}` : ``}/version`, {
      method: 'GET',
			headers: {
    ...
    this.socketUri = `ws${this.secure ? 's' : ''}://${this.host ? this.host : 'localhost'}${this.port ? `:${this.port}` : ':443'}${(this.version as string).replace(/\./g, '') >= '370' ? (this.version as string).replace(/\./g, '') >= '400' ? "/v4/websocket" : "/v3/websocket" : ""}`
  this.restUri = `http${this.secure ? 's' : ''}://${this.host ? this.host : 'localhost'}${this.port ? `:${this.port}` : ':443'}${(this.version as string).replace(/\./g, '') >= '370' ? (this.version as string).replace(/\./g, '') >= '400' ? "/v4/" : "/v3/" : ""}`
  ...
  switch (payload.op) {
      ...
      case "playerUpdate":
        ...
        current[payload.guildId] = {
          ...
          thumbnail: current.thumbnail || `https://img.youtube.com/vi/${current.identifier}/sddefault.jpg`,
      ...
      default:
        this.manager.emit(
          "nodeError",
          this,
          new Error(`[ @Moonlink/Nodes ]: Unexpected op "${payload.op}" with data: ${payload}`)
        )
    }
	}
	...
	protected async handleEvent(payload: any): Promise<any> {
		...
		let player: MoonlinkPlayer = new MoonlinkPlayer(this.map.get('players')[payload.guildId], this.manager, this.map, this.rest);
    ...
    switch (payload.type) {   
      case 'TrackStartEvent': {
        ...
        if (track && player.loop) {
          if (player.loop == 1) {
            if((this.version as string).replace(/\./g, '') <= '370') this.sendWs({
              op: 'play',
              track: track.track,
			...
			guildId: payload.guildId,
      data: {
				encodedTrack: track.track ? track.track : track.encoded ? track.encoded : track.trackEncoded ? track.trackEncoded : null,
			}
		})
return;
          }
...
this.manager.emit("trackEnd", player, track);
        ...
        let req: SearchResult = await this.manager.search(uri);
				if (!req ||!req.tracks ||["LOAD_FAILED", "NO_MATCHES"].includes(req.loadType)) return player.stop();
				let data: any = req.tracks[Math.floor(Math.random() * Math.floor(req.tracks.length))];
