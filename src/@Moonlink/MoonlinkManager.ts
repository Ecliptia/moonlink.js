...
export class MoonlinkManager extends EventEmitter {
  ...
  constructor(
    ...
    if(nodes && !Array.isArray(nodes)) throw new Error('[ @Moonlink/Manager ]: the "nodes" option has to be in an array')
		if(nodes && nodes.length == 0) throw new Error('[ @Moonlink/Manager ]: there are no parameters with "node(s)" information in the object')
		if(options && typeof options.shards !== 'number' && typeof options.shards !== 'undefined') throw new Error('[ @Moonlink/Manager ]: shards option of the "options" parameter must be in number format')
		if(options && typeof options.clientName !== 'string' && typeof options.clientName !== 'undefined') throw new Error('[ @Moonlink/Manager ]: clientName option of the "options" parameter must be in string format')
    this._nodes = nodes;
   ...
   return this;
    ...
    if((this.leastUsedNodes.version as string).replace(/\./g, '') <= '370') this.leastUsedNodes.sendWs({
      op: 'voiceUpdate'
      ...
      , event: voiceServer[guildId].event
		...
		let a: any;
	if((this.leastUsedNodes.version as string).replace(/\./g, '') >= '370') a = await this.leastUsedNodes.rest.update({ 
		guildId, 
		data: {
			voice: {
				sessionId: voiceStates[guildId].session_id, 
