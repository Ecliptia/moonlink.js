...
export class Spotify {
  ...
  public async requestToken(): Promise<void> {
    ...
    let res: any = await makeRequest("https://accounts.spotify.com/api/token?grant_type=client_credentials", {
      method: "POST",
			headers: {
  ...
  public async request(endpoint: string): Promise<any> {
    ...
    let res = await makeRequest(`https://api.spotify.com/v1${/^\//.test(endpoint) ? endpoint : `/${endpoint}`}`,
      {
  ...
  public async fetch(query: string): Promise<any> {
    ...
    let res: any = await this.manager.search(`${track.artists ? track.artists[0]?.name : "Unknown Artist"} ${track.name}`)
