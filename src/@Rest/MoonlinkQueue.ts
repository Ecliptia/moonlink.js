...
export class MoonlinkQueue {
  ...
  public get size(): BigInt {  
	...
	public remove(position: number): boolean {
	  if(!position && typeof position !== 'number') throw new Error('[ @Moonlink/Queue ]: option "position" is empty or different from number');
		if(!this.size) throw new Error('[ @Moonlink/Queue ]: the queue is empty');
		let queue = this.db.get(`queue.${this.guildId}`);
    if(!queue[position - 1]) throw new Error('[ @Moonlink/Queue ]: indicated position is undefined');
