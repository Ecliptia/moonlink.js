...
let node = {
    ...
    , getInfos: function(no) {
        if(!no) throw new TypeError('[ moonlink.js ]: the option to get information from a node has to be filled')
        let node = MemoryNode[no.identifier || `${no.host}${no.port ? ':' + no.port : ''}`] || null
        if(!node) return null 
        return node
    }
	, get: function () {
...
const editSendDs = (object) => sendDiscord = object;
const sendDs = () => sendDiscord;

let makeRequest = function(url, method, opts) {
        return new Promise((resolve) => {
        if(!method) method || 'GET'
        let url_ = new URL(url)
        opts.headers['User-Agent'] = 'moon' 
...
function request(node, endpoint, params) {
	...
	const debug = (event, data) => event.emit(data);
}
