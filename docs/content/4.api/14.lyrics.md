...
#description
Guidelines for effectively using the Lyrics class.

#content
1. **NodeLink Compatibility**
   - Always check if NodeLink is available before attempting to fetch lyrics
   - Handle cases where lyrics are not available gracefully

2. **Performance**
   - Cache lyrics results when appropriate
   - Use pagination for long lyrics in chat interfaces
   - Consider rate limiting for lyrics requests

3. **User Experience**
   - Provide loading indicators while fetching lyrics
   - Handle both synchronized and unsynchronized lyrics appropriately
   - Implement fallback displays when lyrics are unavailable
...
5. **Display Formatting**
   ...
   - Consider text direction (RTL support)
   - Break up long lyrics into manageable chunks
::
