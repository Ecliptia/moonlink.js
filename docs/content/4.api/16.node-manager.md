...
| `id` | `number` | No | - | Numeric identifier for the node |
...
#description
Guidelines for effectively using the NodeManager.

#content
1. **Node Configuration**
   - Always provide unique identifiers for nodes
   - Use secure connections for production environments
   - Configure appropriate retry settings
   - Set up regional nodes for better performance

2. **Load Balancing**
   - Use multiple nodes for redundancy
   - Configure regions for optimal routing
   - Monitor node usage with `sortByUsage`
   - Distribute players across nodes

3. **Error Handling**
   - Listen for node events
   - Implement reconnection logic
   - Handle node failures gracefully
   - Log important node events
