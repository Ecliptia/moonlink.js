...
module.exports = {
  ...
  execute(message, args, client) {
    ...
    // Step 3: Check if there are any tracks in the queue or currently playing.
    if (!player.current && player.queue.size === 0) {
      return message.reply('There are no tracks in the queue!');
    }
    
    // Step 4: Create a helper function to format track duration.
    // This converts milliseconds into a more readable format (HH:MM:SS).
    ...
    // Step 5: Create a Discord embed to display the queue.
    // Embeds provide a visually appealing way to present information.
    const embed = new EmbedBuilder()
      .setTitle('Current Queue')
      .setColor('#0099ff');
    
    // Step 6: Add the currently playing track to the embed.
    if (player.current) {
      embed.setDescription(`**Now Playing:**\n[${player.current.title}](${player.current.uri}) | \`${formatDuration(player.current.duration)}\``);
    }
    
    // Step 7: Add the next tracks in the queue to the embed.
    if (player.queue.size > 0) {
      const tracks = player.queue.tracks.map((track, index) => {
        return `${index + 1}. [${track.title}](${track.uri}) | \`${formatDuration(track.duration)}\``;
      });
      
      // We'll show up to 10 tracks in the queue.
      embed.addFields({
        name: 'Up Next:',
        value: tracks.slice(0, 10).join('\n'),
      });
      
      // If there are more than 10 tracks, we'll add a note.
    ...
    // Step 8: Send the embed as a reply to the user.
