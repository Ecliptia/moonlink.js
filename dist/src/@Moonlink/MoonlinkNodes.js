...
class MoonlinkNode {
    ...
    async connect() {
        ...
        if (this.port && typeof this.port !== 'number' || this.port > 65535 || this.port < 0)
        ...
        this.socketUri = `ws${this.secure ? 's' : ''}://${this.host ? this.host : 'localhost'}${this.port ? `:${this.port}` : ':443'}${this.version.replace(/\./g, '') >= '370' ? this.version.replace(/\./g, '') >= '400' ? "/v4/websocket" : "/v3/websocket" : ""}`;
        this.restUri = `http${this.secure ? 's' : ''}://${this.host ? this.host : 'localhost'}${this.port ? `:${this.port}` : ':443'}${this.version.replace(/\./g, '') >= '370' ? this.version.replace(/\./g, '') >= '400' ? "/v4/" : "/v3/" : ""}`;
        ...
        switch (payload.type) {
            ...
            case 'TrackEndEvent': {
                ...
                if (player.autoPlay) {
                    ...
                    if (!req || !req.tracks || ["LOAD_FAILED", "NO_MATCHES"].includes(req.loadType))
                        return player.stop();
