...
class MoonlinkManager extends node_events_1.EventEmitter {
  ...
  constructor(nodes, options, sPayload) {
    ...
    if (
      ...
      throw new Error(
        '[ @Moonlink/Manager ]: shards option of the "options" parameter must be in number format'
      );
    if (
      options &&
    ...
    return new Promise(async (resolve) => {
      ...
      if (res.loadType === "LOAD_FAILED" || res.loadType === "NO_MATCHES") {
        ...
        const tracks = res.tracks.map(
          (x) => new MoonlinkTrack_1.MoonlinkTrack(x)
        );
  ...
  get players() {
    ...
    let get = (guildId) => {
      ...
      return new MoonlinkPlayers_1.MoonlinkPlayer(
