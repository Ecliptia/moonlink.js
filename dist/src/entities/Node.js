...
class Node {
    ...
    async handleWebSocketClosed(player, payload) {
        ...
        if (!voiceOptions?.autoReconnect || !player.get("userInitiatedConnect")) {
            ...
            if (!player.get("userInitiatedConnect"))
                await player.destroy("Disconnected by user/kick");
            return;
        }
    ...
    async updateRecommendations(player) {
        const track = player.current;
        if (!track || !track.sourceName || !track.identifier)
            return;
        let uri = null;
    ...
    async _resumePlayers() {
        ...
        for (const playerInfo of players) {
            ...
            this.manager.emit("debug", `Moonlink.js > Node > Player ${guildId} has been resumed on node ${this.uuid}.`);
